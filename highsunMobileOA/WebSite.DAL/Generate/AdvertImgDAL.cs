...
namespace WebSite.DAL
{
	...
	{	
        ...
        public int GetMaxID()
        {
            string sql = "select max(ID)+1 from AdvertImg";
        ...
        {
            string sql = "select count(ID) from AdvertImg";
            if (strWhere.Trim() != "")
            {
                sql += " where " + strWhere;
            }
        ...
        {
			string sql = "SELECT ID FROM [AdvertImg] WHERE ID = @ID";
		...
		{
			string sql = "SELECT * FROM [AdvertImg] WHERE ID = @ID";
			SqlParameter[] para = new SqlParameter[]
			{
				new SqlParameter("@ID", iD)
			};
			Database db = new Database(sql,para);
			return db.ExecuteScalar<AdvertImg>();
		}
		...
		{
			string sql = "SELECT * FROM [AdvertImg]";
            if (strWhere.Trim() != "")
            {
                sql += " where " + strWhere;
            }
        ...
        {
			string sql = "SELECT * FROM [AdvertImg] Order by ID Desc";
			Database db = new Database(sql,null);
		...
		/// <returns>返回查询结果实体列表</returns>
		/// </summary>  
		public IList<AdvertImg> SearchBySql( string sql, params SqlParameter[] values )
		{
		...
		{
            string sql ="INSERT [AdvertImg] (AdvertID, Number, Url)" +
			"VALUES (@AdvertID, @Number, @Url);select @@IDENTITY";
			SqlParameter[] para = new SqlParameter[]
			{
         		new SqlParameter("@AdvertID", advertImg.AdvertID),
         		new SqlParameter("@Number", advertImg.Number),
				new SqlParameter("@Url", advertImg.Url)
			};				
			Database db = new Database(sql, para);
		...
		{    
			string sql = "DELETE [AdvertImg] WHERE ID = @ID";
			SqlParameter[] para = new SqlParameter[]
			{
			new SqlParameter("@ID", iD)
			};
		...
		{
			string sql ="UPDATE [AdvertImg] " + "SET " +
				"AdvertID = @AdvertID, " +
				"Number = @Number, " +
				"Url = @Url "  +
			"WHERE ID = @ID";
			...
			SqlParameter[] para = new SqlParameter[]
			{
				new SqlParameter("@ID", advertImg.ID),
				new SqlParameter("@AdvertID", advertImg.AdvertID),
				new SqlParameter("@Number", advertImg.Number),
				new SqlParameter("@Url", advertImg.Url)
			};					
