<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="ERPPrjCompleteReport.aspx.cs" Inherits="highsunMobileOA.ERPPrjCompleteReport" %>
...
<body>
    ...
    <script>
        ...
        $.post('/json/PublicHandler.ashx?Method=SearchERPCOSTComPrjConCompleteReport&ComID='+ComID, function (item) {
            var result = item[0];
            var ProjectCompanyName = item[1];
            var SignedNum = item[2];
            var ContractTotalPrice = item[3];
            var CompletedPrice = item[4];
            var ProjectName = item[5];
            var text = "";
            if (result.length != 0) {
