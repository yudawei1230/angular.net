...
<body >
    ...
    <script>
        ...
        if (URL.indexOf("redoStartDate") > -1) {             //判断，如果是返回，则把返回的redoDate覆盖date，并改变select
            var redoDate = URL.split("=")[1];
            date = redoDate;
            var redoStartYear = URL.split("=")[1].split("-")[0];
            var redoStartMonth = URL.split("=")[1].split("-")[1];
            var redoStartDay = URL.split("=")[1].split("&")[0].split("-")[2];
            var redoEndYear = URL.split("=")[2].split("-")[0];
            var redoEndMonth = URL.split("=")[2].split("-")[1];
            var redoEndDay = URL.split("=")[2].split("-")[2];
            var dateStart = redoStartYear + '-' + redoStartMonth + '-' + redoStartDay;
            var dateEnd = redoEndYear + '-' + redoEndMonth + '-' + redoEndDay;
            if (redoStartMonth < 10) {
                redoStartMonth = redoStartMonth.substring(1, 2);
            };
            if (redoStartDay < 10) {
                redoStartDay = redoStartDay.substring(1, 2);
            };
            if (redoEndMonth < 10) {
                redoEndMonth = redoEndMonth.substring(1, 2);
            };
            if (redoEndDay < 10) {
                redoEndDay = redoEndDay.substring(1, 2);
            };
            $('#yearStart').val(redoStartYear);
            $('#monthStart').val(redoStartMonth);
            $('#dayStart').val(redoStartDay);
            $('#yearEnd').val(redoEndYear);
            $('#monthEnd').val(redoEndMonth);
            $('#dayEnd').val(redoEndDay);
        }
        $.post('/json/PublicHandler.ashx?Method=SearchERPSellHouseClueAnalysisReport&dateStart=' + dateStart + '&dateEnd=' + dateEnd, function (item) {
            var text = "";
            var result = item[0];
