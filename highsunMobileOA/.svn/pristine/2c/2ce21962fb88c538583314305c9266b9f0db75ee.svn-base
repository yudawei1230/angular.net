...
namespace WebSite.DAL
{
	...
	{	
        ...
        public int GetMaxID()
        {
            string sql = "select max(ID)+1 from MediaSource";
        ...
        {
            string sql = "select count(ID) from MediaSource";
            if (strWhere.Trim() != "")
            {
                sql += " where " + strWhere;
            }
        ...
        {
			string sql = "SELECT ID FROM [MediaSource] WHERE ID = @ID";
		...
		{
			string sql = "SELECT * FROM [MediaSource] WHERE ID = @ID";
			SqlParameter[] para = new SqlParameter[]
			{
				new SqlParameter("@ID", iD)
			};
			Database db = new Database(sql,para);
			return db.ExecuteScalar<MediaSource>();
		}
		...
		{
			string sql = "SELECT * FROM [MediaSource]";
            if (strWhere.Trim() != "")
            {
                sql += " where " + strWhere;
            }
        ...
        {
			string sql = "SELECT * FROM [MediaSource] Order by ID Desc";
			Database db = new Database(sql,null);
		...
		/// <returns>返回查询结果实体列表</returns>
		/// </summary>  
		public IList<MediaSource> SearchBySql( string sql, params SqlParameter[] values )
		{
		...
		{
            string sql ="INSERT [MediaSource] (Name, IsDel, IsSort)" +
			"VALUES (@Name, @IsDel, @IsSort);select @@IDENTITY";
			SqlParameter[] para = new SqlParameter[]
			{
         		new SqlParameter("@Name", mediaSource.Name),
         		new SqlParameter("@IsDel", mediaSource.IsDel),
				new SqlParameter("@IsSort", mediaSource.IsSort)
			};				
			Database db = new Database(sql, para);
		...
		{    
			string sql = "DELETE [MediaSource] WHERE ID = @ID";
			SqlParameter[] para = new SqlParameter[]
			{
			new SqlParameter("@ID", iD)
			};
		...
		{
			string sql ="UPDATE [MediaSource] " + "SET " +
				"Name = @Name, " +
				"IsDel = @IsDel, " +
				"IsSort = @IsSort "  +
			"WHERE ID = @ID";
			...
			SqlParameter[] para = new SqlParameter[]
			{
				new SqlParameter("@ID", mediaSource.ID),
				new SqlParameter("@Name", mediaSource.Name),
				new SqlParameter("@IsDel", mediaSource.IsDel),
				new SqlParameter("@IsSort", mediaSource.IsSort)
			};					
