...
class MainController extends FrontController {
    ...
    public function report(){
		...
		for ($row = 4; $row <= $highestRow; $row++){//行数是以第1行开始
			$table_val[$row]['0'] = $sheet->getCell('B'.$row)->getValue();
			$table_val[$row]['1'] = $sheet->getCell('A'.$row)->getValue();
			$table_val[$row]['2'] = trim($sheet->getCell('D'.$row)->getValue());
		}
    ...
    public function editreport(){
		...
		*/
		...
		file_put_contents(TEMP_PATH.$txt_filename, '');
     	...
     	$data['path'] = date('Y-m-d').'/'.$this->user['id'].'-'.$year.'-'.$month.'-'.$frequentness.'.zip';
     	$data['u_id'] = $this->user['id'];
     	$data['year'] = $year;
		...
		if ($v_list == 0) {
			...
			$report_db = M('Report');
			$tmp = $report_db ->where(array('u_id'=>$this->user['id'],'year'=>$year,'month'=>$zip_month,'frequentness'=>$frequentness))->find();
			if($tmp){
				$a = $report_db ->where(array('u_id'=>$this->user['id'],'year'=>$year,'month'=>$zip_month,'frequentness'=>$frequentness))->data($data)->save();
			}else{
